...
procedure Show_Type_Invariant is
   ...
   package body Courses is
      ...
      begin
         return
           Course'
             (Name     => To_Unbounded_String (Name), Start_Date => Start_Date,
              End_Date => End_Date);
      end Init;
   end Courses;
...
begin
   Add
     (CC,
      Init
        (Name => "Intro to Photography", Start_Date => Time_Of (2_018, 5, 1),
         End_Date => Time_Of (2_018, 5, 10)));

   Add
     (CC,
      Init
        (Name       => "Intro to Video Recording",
         Start_Date => Time_Of (2_019, 5, 1),
         End_Date   => Time_Of (2_018, 5, 10)));
